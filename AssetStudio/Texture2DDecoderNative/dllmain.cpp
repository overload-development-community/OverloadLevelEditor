...
T2D_API(void *) UnpackUnityCrunchInit(const void* hdr, uint32_t hdrSize, int level_index, uint32_t* pDataOfs, uint32_t* pDataSize, uint32_t* pResultSize)
{
	return unity_crunch_unpack_init(hdr, hdrSize, level_index, pDataOfs, pDataSize, pResultSize);
}

T2D_API(int) UnpackUnityCrunchLevelData(void* pContext, int level_index, const void* pData, uint32_t pDataSize, void *pDst, int pDstSize)
{
	return unity_crunch_unpack_level_data(pContext, level_index, pData, pDataSize, pDst, pDstSize) ? 1 : 0;
}

T2D_API(void) UnpackUnityCrunchDone(void *pContext)
{
	unity_crunch_unpack_done(pContext);
}
