...
namespace Texture2DDecoder
{
    public static unsafe partial class TextureDecoder
    {
        ...
        public static IntPtr UnpackUnityCrunchInit(byte[] hdr, int level_index, out uint dataOfs, out uint dataSize, out uint resultSize)
		{
			fixed (byte *pHdr = hdr)
				return UnpackUnityCrunchInit(pHdr, (uint)hdr.Length, level_index, out dataOfs, out dataSize, out resultSize);
		}

        public static byte[] UnpackUnityCrunchLevelData(IntPtr context, int level_index, byte[] data, uint resultSize)
		{
			var result = new byte[resultSize];
			int ret;
			fixed (byte *pData = data)
				fixed (byte *pResult = result)
					ret = UnpackUnityCrunchLevelData(context, level_index, pData, (uint)data.Length, pResult, resultSize);
