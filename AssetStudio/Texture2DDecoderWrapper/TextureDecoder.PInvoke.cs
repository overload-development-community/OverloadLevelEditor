...
namespace Texture2DDecoder
{
    unsafe partial class TextureDecoder
    {
        ...
        [DllImport(T2DDll.DllName, CallingConvention = CallingConvention.Winapi)]
        private static extern IntPtr UnpackUnityCrunchInit(void* hdr, uint hdrSize, int level_index, out uint dataOfs, out uint dataSize, out uint resultSize);

        [DllImport(T2DDll.DllName, CallingConvention = CallingConvention.Winapi)]
        private static extern int UnpackUnityCrunchLevelData(IntPtr context, int level_index, void* data, uint dataSize, void* result, uint resultSize);

        [DllImport(T2DDll.DllName, CallingConvention = CallingConvention.Winapi)]
        public static extern void UnpackUnityCrunchDone(IntPtr context);
    }
}
