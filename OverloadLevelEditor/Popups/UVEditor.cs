...
namespace OverloadLevelEditor
{
	public partial class UVEditor : Form
	{
		...
		public int[] selected_uv = { -1, -1 }; // Side/Vert
		...
		private void view_MouseDown(object sender, MouseEventArgs e)
		{
			...
			switch (e.Button) {
				...
				case MouseButtons.Right:
					editor.SaveStateForUndo("Move UVs");
					m_cancel_move = false;
					break;
			}
		...
		public void UVEditor_FormClosing(object sender, FormClosingEventArgs e)
		{
		...
		private void UVEditor_KeyDown(object sender, KeyEventArgs e)
		{
			switch (e.KeyCode) {
				...
				case Keys.G:
					if (e.Control) {
						editor.SaveStateForUndo("Snap UVs");
						SnapMarkedVerts();
					}
					break;
				case Keys.NumPad8:
		...
		{
			Side s = editor.m_level.GetSelectedSide();
			if (s != null && s.m_tex_gl_id > -1) {
				int tex_idx = editor.tm_level.FindTextureIndexByGLID(s.m_tex_gl_id);
            SetTexture(editor.tm_level.m_bitmap[tex_idx]);
			}
		}
		...
		private void UVEditor_Shown(object sender, EventArgs e)
		{
			...
			Location = editor.m_uv_editor_loc;
		}
	}
}
