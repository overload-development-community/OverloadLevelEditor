...
namespace OverloadLevelEditor
{
	...
	public partial class TunnelBuilder : EditorDockContent
	{
		public TunnelMode m_tunnel_mode = TunnelMode.TUNNEL;

		// When changing defaults, you should also change them in the designer (aka the default text)
		public int m_segments = 10;
		public int m_seg_length = 4;
		public int m_arc_segments = 20;
		
		public int m_radius_xy = 20;
		...
		// Scale start X/Y are not actually editable (always 100%)
		...
		public ArcDirection m_arc_direction = ArcDirection.RIGHT;

		public int m_twists = 0;
		public int m_handle_selected = 10;
		public int m_handle_marked = 10;
		public float m_connect_seg_size = 5f;

		public ScalingMode m_scaling_mode = ScalingMode.LINEAR;

		public TunnelBuilder(EditorShell shell)
			: base( shell )
		{
			InitializeComponent();
		}

		private void TunnelBuilder_Load( object sender, EventArgs e )
		{
			UpdateVisibility();
		}
		...
		public void UpdateVisibility()
		{
			...
			label_arc_direction.Enabled = on_arc;
		...
		public void UpdateLabels()
		{
			...
			label_arc_direction.Text = "Arc Direction: " + m_arc_direction.ToString();
			...
			slider_radius_x.ValueText = m_radius_xy.ToString();
		...
		{
			m_handle_selected = Utility.Clamp(m_handle_selected + e.Increment * 1, 1, 50);
			UpdateLabels();
		}

		private void slider_handle_marked_Feedback(object sender, SliderLabelArgs e)
		{
			m_handle_marked = Utility.Clamp(m_handle_marked + e.Increment * 1, 1, 50);
			UpdateLabels();
		}

		private void slider_connect_seg_size_Feedback(object sender, SliderLabelArgs e)
		{
			m_connect_seg_size = Utility.SnapValue(Utility.Clamp(m_connect_seg_size + e.Increment * 0.5f, 0.5f, 15f), 0.5f);
			UpdateLabels();
		}
		...
		public void GenerateTunnel()
		{
			...
			Side start_side = level.GetSelectedSide();
			float total_dist;
			...
			level.UnTagAllVertices();
			for (int i = 0; i < m_segments; i++) {
				level.InsertMarkedSidesMulti(m_seg_length, start_side);
			}

			level.JoinSidesWithTaggedVertices();

			// Apply the modifiers
			List<Vertex> vert_list = level.GetTaggedVerts();
				...
				if (m_tunnel_mode == TunnelMode.ARC) {
					...
					switch (m_arc_direction) {
						...
						case ArcDirection.RIGHT:
							...
							v_offset.X = (1.0f - v_offset.X) * m_radius_xy;
						...
						case ArcDirection.LEFT:
							...
							v_offset.Z *= -m_radius_z;
							break;
						case ArcDirection.DOWN:
							v_pos = Vector3.Transform(v_pos, Matrix4.CreateRotationX(Utility.RAD_360 * rel_dist));
							v_offset = Vector3.Transform(Vector3.UnitY, Matrix4.CreateRotationX(-Utility.RAD_360 * rel_dist));
							v_offset.Y = (1.0f - v_offset.Y) * -m_radius_xy;
							v_offset.Z *= -m_radius_z;
							break;
			...
			List<Side> side_list = level.GetSidesWithTaggedVertices();
		...
		public void GenerateConnection()
		{
			...
			if (level.num_marked_sides == 1) {
				...
				Side start_side = level.GetMarkedSides()[0];
				Side end_side = level.GetSelectedSide();
		...
		void FindHermiteCurveMovementPlaneInfo(Utility.HermiteCurveEvaluator curve, out Vector3 plane_center_pt, out Vector3 plane_normal)
		{
			...
			plane_normal = Vector3.Cross(center_to_end, center_to_start);
			
			//If vectors are parallax cross product will be zero, so artibrarily pick an up vector in the perpendicular plane
			if (plane_normal.Length == 0.0f) {
				Vector3 v = new Vector3(0f, 0f, 1f);
				plane_normal = Vector3.Cross(center_to_end, v);
				if (plane_normal.Length == 0.0f) {
					v.X = 1f;
					v.Z = 0f;
					plane_normal = Vector3.Cross(center_to_end, v);
		...
		Vector3 RotateDirAroundZ(Vector3 from, Vector3 to, float t, int twists)
		{
			...
			else {      //no twists, so twist whichever way is shortest
				float delta_rot = to_rot - from_rot;
		...
		public void GenerateConnectedTunnel(Side s1, Side s2, int[] v1, int[] v2)
		{
			...
			//Adjust the max segment length so that all segments are the same size
			float total_dist = center_curve_evaluator.CalculateDistance(0.0f, 1.0f);
			float num_segs = (float)Math.Ceiling(total_dist / max_segment_length);
			max_segment_length = total_dist / num_segs;
			...
			calculate_side_corners = (float t, Vector3 t_pos) =>
												 {
													 ...
													 for (int i = 0; i < 4; ++i) {
														 ...
														 Vector3 dir_to_corner_in_side_space = RotateDirAroundZ(start_corner_dir, end_corner_dir, t, this.m_twists);
			...
			for (int i = 0; i < num_segments_to_create; i++) {
				last_inserted_segment = level.InsertMarkedSidesMulti((int)(m_connect_seg_size + 0.5f), s1);
			}
			level.JoinSidesWithTaggedVertices();
			...
			if (last_inserted_segment != null && last_inserted_segment.Count == 1) {
				...
				level.JoinSides(level.segment[seg_idx].side[side_idx], s2);		//This function will move first side but leave second side where it is.
			}
		}
		...
		public void GenerateConnectedTunnelOLD(Side s1, Side s2, int[] v1, int[] v2)
		{
			...
			for (int i = 0; i < segments; i++) {
				level.InsertMarkedSidesMulti(seg_length, s1);
			}
			...
			for (int i = 0; i < vert_list.Count; i++) {
				...
				for (int j = 0; j < 4; j++) {
					if ((start_pos - level.vertex[s1.vert[v1[j]]].position).Length < 0.1f) {
						end_pos = level.vertex[s2.vert[v2[j]]].position;
					}
				}
		...
		public void FindClosestVerts(Side s1, Side s2, int[] v1, int[] v2)
		{
			...
			for (int i = 0; i < 4; i++) {
				for (int j = 0; j < 4; j++) {
					dist_sq = (level.vertex[s1.vert[i]].position - level.vertex[s2.vert[j]].position).LengthSquared;
		...
		public void TunnelBuilder_FormClosing(object sender, FormClosingEventArgs e)
		{
		...
		private void label_arc_direction_MouseDown(object sender, MouseEventArgs e)
		{
			m_arc_direction = (ArcDirection)(((int)m_arc_direction + 1) % (int)ArcDirection.NUM);
			UpdateLabels();
		}
		...
		private void button_reset_Click(object sender, EventArgs e)
		{
			...
			m_twists = 0;
			m_handle_selected = 15;
			m_handle_marked = 15;
			m_connect_seg_size = 5f;

			m_scaling_mode = ScalingMode.LINEAR;
			UpdateLabels();
		}
	}
}
